...
export default function Home({
  ...
  // State to track when DigitalConstruction is in viewport
  ...
  const flexibleComponents = data?.acf?.flexible_components;
      ...
      const fetchData = async () => {
        try {
          ...
          if (fetchedProjects && fetchedProjects.length > 0) {
            ...
            setSectorProjectsMap({}); // Reset map
          }
  ...
  useEffect(() => {
    if (selectedSector !== null) {
      // Find the index of the selected sector in the sectors array to key into sectorProjectsMap
      // The sectorProjectsMap is keyed by index of sectors array?
      // Wait, in [...slug].js:
      // fetchedSectors.map((sector, index) => { ... newMap[index] = ... })
      // So sectorProjectsMap is keyed by index.
      // But selectedSector is an object.
      // We need to find the index.
      const sectorIndex = sectors.findIndex((s) => s.id === selectedSector.id);

      if (
        sectorIndex !== -1 &&
        sectorProjectsMap[sectorIndex] &&
        sectorProjectsMap[sectorIndex].length > 0
      ) {
        setFilteredProjects(sectorProjectsMap[sectorIndex]);
      } else {
        setFilteredProjects([]);
      }
    } else {
  ...
  const renderComponent = (item, index) => {
    //console.log("Rendering item:", index, item.acf_fc_layout, item);
    const Component = COMPONENT_MAP[item.acf_fc_layout];
    if (!Component) {
      console.warn("No component found for layout:", item.acf_fc_layout);
      return null;
    }
    ...
    if (item.acf_fc_layout === "selected_sectors") {
      const activeSector = "";
      const title = item.swin_title || item.title || "Sectors we work in";
    ...
    // Special handling for VideoContainer props
    ...
    // Special handling for DigitalConstruction callbacks
    ...
    // Special handling for CareerSection
    ...
    // Special handling for LazyLandmarksSlider (Projects Showcase)
      ...
      return (
        <Wrapper key={index}>
          <div
            ...
            className={`${
              isDigitalConstructionInView ? "darkModeSection" : ""
            } pt_80 pb_80`}
          >
            {mountLandmarks ? (
    ...
    // Special handling for ServiceList
    if (item.acf_fc_layout === "service_list") {
      return (
        <Wrapper key={index}>
          <ServiceList
    ...
    // Special handling for ServiceList with Dark Background
    if (
      item.acf_fc_layout === "services_list_with_dark_background" ||
      item.acf_fc_layout === "layout_services_list_with_dark_background"
    ) {
      return (
        <Wrapper key={index}>
          <ServiceList
    ...
    if (item.acf_fc_layout === "support_stories") {
      return (
        <Wrapper key={index}>
          <SupportStories
            title={item.ep_bb_title ? parse(String(item.ep_bb_title)) : ""}
            description={[
              item.ep_bb_description
                ? parse(String(item.ep_bb_description))
                : "",
            ]}
            image={item.ep_bb_image?.url || "/images/stories-1.png"}
          />
        </Wrapper>
      );
    }
    ...
    if (item.acf_fc_layout === "success_stories") {
      return (
        <Wrapper key={index}>
          <div data-aos="fade-up" data-aos-delay="600">
            <SuccessStories
              title={item.cd_ss_title}
              description={item.cd_ss_short_description}
              stories={item.cd_ss_stories}
            />
          </div>
        </Wrapper>
      );
    ...
    // Special handling for Quality Policy
    if (item.acf_fc_layout === "quality_policy") {
      return (
        <Wrapper key={index}>
          <QualityPolicy
    ...
    // Special handling for ReachOutTeam
    if (item.acf_fc_layout === "reach_out_team") {
      return (
        <Wrapper key={index}>
          <div data-aos="fade-up" data-aos-delay="600">
    ...
    // Special handling for LocationMap
    if (item.acf_fc_layout === "location_map") {
      return (
        <Wrapper key={index}>
          <div data-aos="fade-up" data-aos-delay="600">
    ...
    // Special handling for Red Feature Block
...
export async function getStaticProps() {
  try {
    const response = await axiosServer.get(
      `/pages?slug=${constants.HOME}&acf_format=standard&_embed`,
    );
    ...
    if (pageData && pageData.length > 0) {
      ...
      // Fetch featured media details if available
      if (page.featured_media) {
        try {
          const mediaResponse = await axiosServer.get(
            `/media/${page.featured_media}?_fields=id,source_url`,
          );
          page.featured_media_details = mediaResponse.data;
        } catch (mediaError) {
          console.error("Error fetching featured media:", mediaError);
          page.featured_media_details = null;
        }
      }

      // Ensure page.acf exists
      if (!page.acf) {
        console.warn("page.acf is missing, creating empty object");
        page.acf = {};
      }

      // 1. Normalize flexible_components: If missing, construct from top-level fields (Legacy/Fallback)
      if (
        !page.acf.flexible_components ||
        page.acf.flexible_components.length === 0
      ) {
        // console.log(
        //   "Constructing fallback flexible_components from top-level fields",
        // );
        page.acf.flexible_components = [
          {
            acf_fc_layout: "home_banner",
            hm_banner_video: page.acf.hm_banner_video,
            hm_banner_bg: page.acf.hm_banner_bg,
            hm_banner_text: page.acf.hm_banner_text,
          },
          {
            acf_fc_layout: "building_showcase",
            hm_bs_title: page.acf.hm_bs_title,
            hm_bs_description: page.acf.hm_bs_description,
            hm_bs_left_image: page.acf.hm_bs_left_image,
            hm_bs_right_image: page.acf.hm_bs_right_image,
            hm_bs_button: page.acf.hm_bs_button,
          },
          {
            acf_fc_layout: "capabilities",
            hm_oc_title: page.acf.hm_oc_title,
            hm_oc_button: page.acf.hm_oc_button,
            hm_oc_description: page.acf.hm_oc_description,
            hm_oc_our_services: page.acf.hm_oc_our_services,
          },
          {
            acf_fc_layout: "projects_showcase",
            hm_ps_sub_caption: page.acf.hm_ps_sub_caption,
            hm_ps_main_caption: page.acf.hm_ps_main_caption,
            hm_projects_list: page.acf.hm_projects_list,
            hm_ps_button: page.acf.hm_ps_button,
          },
          {
            acf_fc_layout: "digital_construction",
            hm_bb_main_image: page.acf.hm_bb_main_image,
            hm_bb_main_image_mob: page.acf.hm_bb_main_image_mob,
            hm_bb_sub_caption: page.acf.hm_bb_sub_caption,
            hm_bb_main_caption: page.acf.hm_bb_main_caption,
            hm_bb_description: page.acf.hm_bb_description,
            hm_bb_button: page.acf.hm_bb_button,
          },
          {
            acf_fc_layout: "career_section",
            hm_li_image: page.acf.hm_li_image,
            hm_li_sub_caption: page.acf.hm_li_sub_caption,
            hm_li_main_caption: page.acf.hm_li_main_caption,
            hm_li_button: page.acf.hm_li_button,
            hm_li_description: page.acf.hm_li_description,
          },
          {
            acf_fc_layout: "video_section",
            videoUrl: page.acf.hm_vs_video,
            thumbnail: page.acf.hm_vs_background_image_,
          },
          {
            acf_fc_layout: "whats_new",
            hm_wn_latest_news: page.acf.hm_wn_latest_news,
            hm_wn_button: page.acf.hm_wn_button,
          },
        ];
      }
      ...
      if (
        ...
        const hasTeamGrid = page.acf.flexible_components.some(
          (comp) =>
      ...
      // 2. Hydrate flexible_components data (Fetch details for relations)
      if (
        page.acf.flexible_components &&
        page.acf.flexible_components.length > 0
      ) {
        for (const component of page.acf.flexible_components) {
          ...
          if (
            (component.acf_fc_layout === "capabilities" ||
              component.acf_fc_layout === "layout_service_list" ||
          ...
          if (
            ...
            try {
              ...
              if (ids.length > 0) {
                // Fetch taxonomies for correct tag mapping
                const [sectorsRes, categoriesRes] = await Promise.all([
                  axiosServer
                    .get("/sector?per_page=100")
                    .catch(() => ({ data: [] })),
          ...
          if (
            (component.acf_fc_layout === "layout_whats_new" ||
          ...
          // E. Hydrate Red Feature Block (Digital Workflows)
          if (
            component.acf_fc_layout === "red_feature_block" ||
            component.acf_fc_layout === "digital_work_flows"
          ) {
            try {
              if (
      ...
      const pageScripts = {
        header_scripts: page.meta?._hfs_header_scripts || "",
        body_scripts: page.meta?._hfs_body_scripts || "",
        footer_scripts: page.meta?._hfs_footer_scripts || "",
      };
